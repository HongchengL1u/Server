    ...
    public:
        ...
        void make_new_connection(Socket& listen_socket)
        {
            ...
            while(1)
            {
                ...
                connect_socket_ptr->set_nonnagle();
                struct epoll_event new_ev;
                Channel msg_process_event;
                // 对于RAII的目标，谨慎使用move
                auto func = std::bind(&Epoll::msg_callback, this, connect_socket_ptr);
                msg_process_event.bind(connect_fd, std::move(func));
                // ET模式时，事件就绪时，假设对事件没做处理，内核不会反复通知事件就绪
                new_ev.events = EPOLLET | EPOLLIN;
                // EPOLLOUT 缓冲区可读
                map.emplace(connect_fd, msg_process_event);
                new_ev.data.ptr = &(map[connect_fd]);
                // ev如果被挂载到epfd上，则其上的内容是会被保存的
                if(epoll_ctl(event_fd,EPOLL_CTL_ADD,connect_fd,&new_ev)==-1)
                {
                    LOG_FATAL << "epoll create fail!";
                }
            }
        }
        ...
        bool create(Socket& listen_socket)
        {
            ...
            Channel accept_event;
            auto func = std::bind(&Epoll::make_new_connection, this, std::ref(listen_socket));
            accept_event.bind(listen_sock_fd, std::move(func));
            // 文件描述符可读
            // ET模式时，事件就绪时，假设对事件没做处理，内核不会反复通知事件就绪
            ev.events = EPOLLIN | EPOLLET; 
            map.emplace(listen_sock_fd, accept_event);
            ...
            while(1)
            {
                ...
                else
                {
                    for(int i=0;i<event_nums;i++)
                    {
                        ...
                        int ev_fd = ((Channel*)(events[i].data.ptr))->get_fd();
                        ...
                        pool.addfunc(((Channel*)(events[i].data.ptr))->get_func());
